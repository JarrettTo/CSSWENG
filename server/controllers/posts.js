...
export const getRegisteredPosts = async (req, res) => {
  ...
  try {
    const posts = await PostMessage.find({
      _id: { $in: req.query.regpostsquery.split(",") },
    });
    console.log("finding");
    console.log(posts);
    res.json({ data: posts });
...
const updatePostStatus = async (posts) => {
  ...
  for (var i = 0; i < posts.length; i++) {
    ...
    if (posts[i].expiryDate - date <= 0) {
      //if current date is past expiry date set status to dateClosed
      posts[i].status = "dateClosed";
      await PostMessage.findByIdAndUpdate(posts[i]._id, posts[i], {
        new: true,
      });
    }
  }
...
export const updatePost = async (req, res) => {
  ...
  const updatedPost = await PostMessage.findByIdAndUpdate(
    ...
    { ...post, _id },
    { new: true }
  ); //find post in database with passed id and update
...
export const togglePost = async (req, res) => {
  ...
  if (post.status == "Open") {
    //if post was formerly open, set status to manua        l          aaaly closed
    post.status = "manualClosed";
  } else if (post.expiryDate - date > 0) {
    //if it isnt open and date is not past expiration date, set it to open
    post.status = "Open";
  } else {
    return res
      .status(404)
      .json("Post is past expiration date. Please extend to reopen show");
  }
  if (!mongoose.isValidObjectId(_id))
    return res.status(404).send("no post with that id");
...
export const registerPost = async (req, res) => {
  ...
  const registeredIndex = post.registeredUsers.findIndex(
    (_id) => _id === String(req.id)
  ); //find if user is in show's registeredUsers array
  const acceptedIndex = post.acceptedUsers.findIndex(
    (_id) => _id === String(req.id)
  ); //find if user in show's acceptedUsers array
  ...
  if (registeredIndex === -1 && acceptedIndex === -1 && valid) {
    //check if user has not yet registered and show is still vaalid
    if (foundUser.dlsu && !foundUser.claimed) {
      //check if user is valid art pass holder
    ...
    finalTxn = new form({
      ...
      postID: id,
      postName: post.title,
      selectedFile: register.payment,
      artPass: claim,
      status: status,
      firstName: foundUser.firstName,
      lastName: foundUser.lastName,
      dlsu_id: register.dlsu_id,
      contactNumber: register.contactNumber,
      degree: register.degree,
      college: register.college,
      altClass: register.altClass,
      email: foundUser.email,
      date: Date(post.activeDate),
    });
    ...
    //check if user is already registered and show is still valid
    post.registeredUsers = post.registeredUsers.filter(
      (id) => id !== String(req.id)
    );
    post.acceptedUsers = post.acceptedUsers.filter(
      (id) => id !== String(req.id)
    ); //remove user from show's registeredUsers and acceptedUsers
    foundUser.registeredShows = foundUser.registeredShows.filter(
      (eid) => eid !== id
    ); //remove show from user's registeredShows and acceptedShows
    foundUser.acceptedShows = foundUser.acceptedShows.filter(
      (eid) => eid !== id
    );
    const txn = await form
      .findOne({ userID: req.id, postID: id })
      .sort({ date: -1 }); //find latest transaction between user and show and cancel it
    txn.status = "Cancelled";
    if (txn?.artPass) {
      //check if user registered for the show using art pass
  ...
  if (post.noOfAttendees >= post.maxAttendees) {
    //check if no of attendees exceeds max attendees set by the admin
    post.status = "maxClosed";
  }
  if (post.noOfAttendees < post.maxAttendees && post.status == "maxClosed") {
    //check if no of attendees is less than the max and if the previous status is closed due to full booking
    post.status = "Open";
  }
  ...
  const updatedUser = await user.findByIdAndUpdate(req.id, foundUser, {
    new: true,
  });
