...
export const signUp = async (req, res) => {
  const { email, password, confirmPassword, firstName, lastName, id, dlsu } =
    req.body;
...
export const signIn = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { id: foundUser._id, email: foundUser.email, isAdmin: foundUser.admin },
      "dlsucao",
      { expiresIn: "2h" }
    ); //create jwt token
...
export const googleSign = async (req, res) => {
  ...
  try {
    const foundUser = await user.findOne({ email: email });
    if (foundUser) {
      //check if user exists, if so, generate token for them
      const token = jwt.sign(
        { id: foundUser._id, email: foundUser.email, isAdmin: foundUser.admin },
        "dlsucao",
        { expiresIn: "2h" }
