...
export const getTxn = async (req, res) => {
  ...
  try {
    const txn = await form
      ...
      .sort({ date: -1 }); //looks for all messages with the same model as models/postMessage.js in the database
    console.log(txn);
...
export const approveTxn = async (req, res) => {
  ...
  try {
    ...
    await PostMessage.findByIdAndUpdate(foundPost._id, foundPost, {
      new: true,
    });
