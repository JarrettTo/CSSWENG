...
export const addLog = async (userID, postID, txnID, email, show, date, name)=>{      //function for getting posts
    ...
    try{
        await newLog.save();
        qr=await QRCode.toDataURL(JSON.stringify(newLog), {                     //generates qr code based on new attendanceLog object
			width: 800,
			margin: 2,
			color: {
				dark: '#000000',
        ...
        const transporter = nodemailer.createTransport({                //initialization of emailing transporter
            service: 'gmail',
            auth: {
                type: 'OAuth2',
                user: "justin_jarrett_to@dlsu.edu.ph",
                pass: "yoqIsa103_",
                clientId: "387249647738-f58tonsbl58g3n75hh3rt3mqs9bkl0r0.apps.googleusercontent.com",           //google api service credentials
        ...
        const mailConfigurations = {                                            //email configurations
        ...
        transporter.sendMail(mailConfigurations, function(error, info){                                 //use transporter to send email
...
export const logTime= async (req,res)=>{      
    ...
    if(foundLog){
        if(foundLog.timeIn==null){          //if there is no record of logged time for that particular user and show pairing, log current time as timeIn
