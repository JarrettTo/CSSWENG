...
const PostDetails = () => {
  ...
  const selPost = posts?.find((e) => {
    return e._id == id;
  }); //finds post in posts reducer state based on id
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      //check if non dlsu user filled out required fields
  ...
  return (
    <Grid className={classes.paperhere}>
      <main>
          ...
          <Container className={classes.box1}>
            ...
            <Container className={[classes.innerBox, classes.right]}>
              <Container className={classes.innerRight}>
                <Container>
                  <Typography className={classes.eventBy}>
                    EVENT BY {selPost?.creator}
                  </Typography>
                </Container>
                ...
                <Container className={classes.bottomInfo}>
                  ...
                  <Container className={classes.attendees}>
                    <Typography className={classes.bottomValue}>
                      {selPost?.maxAttendees}
                    </Typography>
                    <Typography className={classes.bottomTitle}>
                      MAX ATTENDEES
                    </Typography>
                    <Typography className={classes.bottomValue}>
                      {selPost?.noOfAttendees}
                    </Typography>
                    <Typography className={classes.bottomTitle}>
                      NUMBER OF ATTENDEES
                    </Typography>
          ...
          <Container className={classes.right2}>
            ...
            <Container className={classes.registration}>
              <Container className={classes.innerReg}>
                ...
                <Typography className={classes.status}>
                  {selPost?.status != "Open"
                    ? "\nThis show is currently not allowing registrations"
                    : null}
                </Typography>
              </Container>
              ...
              selPost?.status == "Open" ? (
                <>
                  {complete ? (
                    <Typography> Please Fill In The Fields</Typography>
                  ) : null}
                    ...
                    {user?.result?.dlsu ? (
                      <>
                        <TextField
                          ...
                          value={form.dlsu_id}
                          onChange={(e) => {
                            setForm({ ...form, dlsu_id: e.target.value });
                          }}
                          className={classes.textField}
                          InputProps={{ className: classes.input }}
                        />
                        ...
                        <TextField
                          ...
                          value={form.altClass}
                          onChange={(e) => {
                            setForm({ ...form, altClass: e.target.value });
                          }}
                          className={classes.textField}
                          InputProps={{ className: classes.input }}
                        />
        ...
        {/* 3RD SECTION */}
        <Container className={classes.box3}></Container>
      </main>
    </Grid>
  );
};
