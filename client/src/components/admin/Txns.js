...
import { Container, AppBar, Paper, Typography, CircularProgress, Divider, TextField, Button, Grid, CssBaseline} from '@material-ui/core';
    ...
    */
    ...
    return(
        ...
        <Grid container className={classes.container} alignItems="stretch" spacing={3}>
            ...
            <Typography className={classes.title} variant="h2">Transactions Page</Typography>  
                <AppBar position="static" className={classes.appbar} >
                    <form className={classes.form}>
                        ...
                        />
                        <Button className={classes.filterButton}  onClick={searchTxn}  variant='contained' >Filter</Button>
                    </form>
            ...
            <Grid item xs={12} sm={12}>
                <Typography className={classes.title} variant="h3">Pending</Typography>  

            </Grid>
            {!txns.length ? <CircularProgress />: (
                txns.map((txn) => txn?.status=="Pending"?( 
            ...
            <Grid item xs={12} sm={12}>
                ...
                <Typography className={classes.title} variant="h3">Accepted</Typography>  

            </Grid>

            {!txns.length ? <CircularProgress />: (
            txns.map((txn) => txn?.status=="Accepted"?( 
                <Grid key={txn._id} item xs={12} sm={6} md={5}>
                    <Txn txn={txn} post={selPost} />
                </Grid>
            ):null)) }
            ...
            <Grid item xs={12} sm={12}>
                <Divider className={classes.divider}></Divider>
                <Typography className={classes.title} variant="h3">Cancelled</Typography>                
            </Grid>

            {!txns.length ? <CircularProgress />: (
            txns.map((txn) => txn?.status=="Cancelled"?( 
                <Grid key={txn._id} item xs={12} sm={6} md={5}>
                    <Txn txn={txn} post={selPost} />
                </Grid>
            ):null)) }
